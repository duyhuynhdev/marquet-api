...
public class FindFriendListAPI extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String parameters = "email,type,friendList,yourSocialId,startIdx,numFriends";
            ...
            if (ResponseController.isSuccess(resultCheckerJSON)) {
                ...
                int numFriends = requestJSON.getInt("numFriends");
                JSONArray friendList = requestJSON.getJSONArray("friendList");
                String yourSocialId = requestJSON.getString("yourSocialId");
                ...
                switch (type) {
                    case 1:
                        result = controller.findFriendsViaFacebook(email, yourSocialId, friendList, startIdx, numFriends);
                        logger.info(LoggerFactory.RESPONSE + result);
                        out.print(result);
                        break;
                    case 2:
                        result = controller.findFriendsViaGoogle(email, yourSocialId, friendList, startIdx, numFriends);
                        logger.info(LoggerFactory.RESPONSE + result);
                        out.print(result);
                        break;
                    case 3:
                        result = controller.findFriendsViaTwitter(email, yourSocialId, friendList, startIdx, numFriends);
                        logger.info(LoggerFactory.RESPONSE + result);
                        out.print(result);
                        break;
                }
